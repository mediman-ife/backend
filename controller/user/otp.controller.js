...
exports.store = async (req, res) => {
  try {
    ...
    tab += ` <p style="font-size:1.1em">Hi,</p><p>Thank you for choosing ${process.end.projectName}. Use the following OTP to forget the Password for Password Security</p>`;
    tab += `<h2 style="background: #00466a;margin: 0 auto;width: max-content;padding: 0 10px;color: #fff;border-radius: 4px;">${newOtp}</h2>`;
    tab += ` <p style="font-size:0.9em;">Regards,<br />${process.end.projectName}</p><hr style="border:none;border-top:1px solid #eee" />`;
    ...
    var mailOptions = {
      ...
      to: req.query.email,
      subject: `Sending Email from ${process.end.projectName} for Password Security`,
      html: tab,
    };
